...
(defn layout
  ...
  [:div.min-h-screen.bg-white
     ...
     [:div.flex.justify-between.h-16
      [:div.flex
       ...
       [:div.hidden.sm:-my-px.sm:ml-6.sm:flex.sm:space-x-8
        ...
        [:a.border-transparent.text-gray-500.hover:border-gray-300.hover:text-gray-700.inline-flex.items-center.px-1.pt-1.border-b-2.text-sm.font-medium {:href "#"} "Event sourcing"]]]


      ;; profile combobox
      #_[:div.hidden.sm:ml-6.sm:flex.sm:items-center
       ...
       [:div.ml-3.relative
        ...
        [:div.origin-top-right.absolute.right-0.mt-2.w-48.rounded-md.shadow-lg.py-1.bg-white.ring-1.ring-black.ring-opacity-5.focus:outline-none {:role "menu" :aria-orientation "vertical" :aria-labelledby "user-menu-button" :tab-index "-1"}
         [:a#user-menu-item-0.block.px-4.py-2.text-sm.text-gray-700 {:href "#" :role "menuitem" :tab-index "-1"} "Your Profile"]
         [:a#user-menu-item-1.block.px-4.py-2.text-sm.text-gray-700 {:href "#" :role "menuitem" :tab-index "-1"} "Settings"]
         [:a#user-menu-item-2.block.px-4.py-2.text-sm.text-gray-700 {:href "#" :role "menuitem" :tab-index "-1"} "Sign out"]]]]
   ...
   [:div.py-10
    [:header
     [:div.max-w-7xl.mx-auto.px-4.sm:px-6.lg:px-8
      #_[:h1.text-3xl.font-bold.leading-tight.text-gray-900 "Dashboard"]]]
